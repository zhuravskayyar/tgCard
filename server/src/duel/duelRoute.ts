...
interface DuelOperations {
  ...
  start(playerId: string, searchId: string): Promise<DuelView>;
}

interface DuelRouteDependencies {
  botToken: string;
...
export async function handleDuelRequest(
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/duel/start") {
      ...
      sendJson(response, 201, await dependencies.duels.start(player.id, body.searchId), headers);
      return;
    }
